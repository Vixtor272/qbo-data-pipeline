...
def export_data_to_postgres(df: DataFrame, **kwargs) -> None:
    ...
    Diseñado para los datos de Customers (qb_customers).
    Evita duplicados con UPSERT automático.
    """
    schema_name = 'raw'
    table_name = 'qb_customers'  # Cambiado para customers
    ...
    if 'payload' in df.columns:
        df['payload'] = df['payload'].apply(lambda x: json.dumps(x))
    else:
        # Si no existe payload, creamos uno con toda la fila serializada
        df['payload'] = df.apply(
    ...
    # Columnas alineadas con Customers
    export_columns = [
        'id',
        'payload',
        'create_time',
